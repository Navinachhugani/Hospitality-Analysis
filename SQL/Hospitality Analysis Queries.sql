...
FROM fact_aggregated_bookings;
-- OR
Select CONCAT(CAST((SUM(Successful_bookings)/SUM(Capacity) * 100) as DECIMAL(10, 2)), "%") as Occupancy from fact_aggregated_bookings;
...
-- OR
   SELECT
   CONCAT(ROUND((SELECT COUNT(*) FROM Fact_bookings WHERE Booking_Status = 'Cancelled') 
            / (SELECT COUNT(*) FROM Fact_bookings) * 100, 0), '%') AS Cancellation_rate;

## 4. Total Booking
Select Count(booking_id) as Total_Booking from fact_bookings;
-- OR
SELECT CONCAT(FORMAT(Count(booking_id) / 1000, 0), 'K') AS Total_Booking
FROM Fact_bookings;
...
-- ##23. How do you calculate the Daily Booked Room Nights (DBRN), which indicates on average how many rooms are booked per day over a specific time period?
SELECT 
    SUM(successful_bookings) / DATEDIFF(MAX(check_in_date), MIN(check_in_date)) AS DBRN
FROM 
    fact_aggregated_bookings;
    
-- 24. How do you calculate the Daily Sellable Room Nights (DSRN), which represents on average how many rooms are available for sale per day over a specific time period?
SELECT 
...
SELECT 
    SUM(fact_aggregated_bookings.successful_bookings) / DATEDIFF(MAX(fact_bookings.checkout_date), MIN(fact_bookings.check_in_date)) AS DURN
